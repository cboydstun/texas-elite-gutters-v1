...
export async function getAnalyticsData(
  ...
  const start =
    dateRange.start || new Date(end.getTime() - 30 * 24 * 60 * 60 * 1000); // 30 days ago
  ...
  const newVisitors = thumbprints.filter(
    (t) => t.firstSeen >= start && t.firstSeen <= end,
  ).length;
  ...
  thumbprints.forEach((thumbprint) => {
    ...
    const visitsInRange = thumbprint.visits.filter(
      (visit: { timestamp: Date; duration?: number; page?: string }) =>
  ...
  const averagePageViewsPerVisit =
    totalVisitors > 0
  ...
  // Fill in the data
  thumbprints.forEach((thumbprint) => {
    thumbprint.visits.forEach((visit: { timestamp: Date; page?: string }) => {
  ...
  const recentVisitors = thumbprints
    .sort(
      (a: IThumbprint, b: IThumbprint) =>
        b.lastSeen.getTime() - a.lastSeen.getTime(),
    )
    .slice(0, 10)
    .map((thumbprint) => {
      // Find the most recent visit
      const lastVisit = thumbprint.visits.sort(
